...
cols.with.ab.zeros = function(my.mat, a=0, b=Inf) {
  ...
  zero_mat <- t(matrix(zeros.per.column))
  low.keep = a <= zero_mat
  high.keep = b >= zero_mat
  i.to.keep = t(matrix(low.keep == high.keep))
  vec_fix <- sum(i.to.keep == TRUE)
  new_matrix <- (my.mat[,i.to.keep])
  if(vec_fix == 1){
...
list.extractor = function(my.list, i.to.keep = 0, i.to.remove = 0) {
  if (i.to.keep != 0) {
    L = my.list[i.to.keep]
  }
  if (i.to.remove != 0) {
